...
export async function getVisitsPerLine() {
  ...
  const visitsPerLine = Object.fromEntries(lineIds.map((id) => [id, 0])) as NumberPerLine;
...
export async function getJourneysPerLine() {
  const journeys = await getStorage().getJourneys();
  const journeysPerLine = Object.fromEntries(lineIds.map((id) => [id, 0])) as NumberPerLine;
